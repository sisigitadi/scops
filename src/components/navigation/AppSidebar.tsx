...
import { 
  ...
  Sun, Moon, Globe, ShieldAlert, Cpu
} from 'lucide-react';
...
export default function AppSidebar({ isOpen, onClose, onOpenExitProtocol }: AppSidebarProps) {
  ...
  const menuStructure: NavItem[] = [
    ...
    { label: (t('nav.mitre') as string) || 'MITRE ATT&CK', to: `${appBasePath}/mitre`, icon: <ShieldAlert size={18} /> },
    ...
    { label: (t('nav.automation') as string) || 'OTOMASI SOAR', to: `${appBasePath}/automation`, icon: <Cpu size={18} />, requiresRoles: [ROLES.ADMIN, ROLES.MANAGER, ROLES.DEMO] },
