...
class PresenceStore {
  constructor() {
    ...
    this.cleanupStatus();
    // Run reaper every 5 minutes to purge dead sessions (>10 mins inactive)
    this.reaperInterval = setInterval(() => this.cleanupStatus(10), 5 * 60 * 1000);
  ...
  async purgeAll() {
    try {
      ...
      console.log('[PRESENCE] Boot Cleanup: Flushed existing presence records.');
    } catch (err) {
      console.error('[PRESENCE] Boot Purge Error:', err.message);
    }
  }

  async cleanupStatus(maxAgeMinutes = 10) {
    try {
      const now = new Date();
      ...
      if (count > 0) console.log(`[PRESENCE] Reaper Pulse: Purged ${count} stale records (> ${maxAgeMinutes}m).`);
    } catch (err) {
