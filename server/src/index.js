...
import { encryptConfigObject, decryptConfigObject } from './utils/cryptoVault.js'
import { loginRateLimiter, apiRateLimiter } from './utils/rateLimiter.js'
...
// Global API rate limiter & request logger
app.use('/api', apiRateLimiter)
app.use(requestLogger)

// Flush any dead presence entries from prior runs on server boot
presenceStore.purgeAll()
...
app.get('/api/config/infra', async (req, res) => {
  try {
    ...
    if (setting) {
      const rawConfig = JSON.parse(setting.value)
      const decrypted = decryptConfigObject(rawConfig)
      res.json(decrypted)
    } else {
      res.json({})
    }
...
app.post('/api/config/infra', requireCapability(CAPABILITIES.MANAGE_INTEGRATION), async (req, res) => {
  try {
    const encryptedBody = encryptConfigObject(req.body)
    const setting = await prisma.systemSetting.upsert({
      where: { key: 'infra_config' },
      update: { value: JSON.stringify(encryptedBody) },
      create: { key: 'infra_config', value: JSON.stringify(encryptedBody) }
    })
    res.json({ success: true, message: 'Configuration securely encrypted and saved.' })
  } catch (error) {
